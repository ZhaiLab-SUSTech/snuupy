...
from .tools import readFasta, FastaContent


def GetConsensusSeq():

    seq2numDict = {"-": 4, "A": 0, "T": 1, "G": 2, "C": 3}
...
def getConsensesFasta(readLs, tempPath, penaltyPath, poaPath, finalPath=False):
    ...
    os.system(
        f"{poaPath} -read_fasta {tempPath}_all.fa -pir {tempPath}_all.aln {penaltyPath}"
    )
...
def getPolishRead(tempPath, finalPath, minimapPath, raconPath, times=1):
    ...
    for x in range(times):
        commandExecuted = _getPolishCommand(
            commandExecuted,
            allPolishedPath[x],
            f"{tempPath}_all.fa",
            allPolishedPath[x + 1],
...
def polishSeq(
    ...
    for read in readLs:
        ...
        if readStrand == "1":
            readSeq = nanoporeDict[readName].getAnti().seq
...
def chunkPolishSeq(
    ...
    nanoporeRead = FastaContent(nanoporeReadPath, True)
    ...
    if i % 100 == 0:
        logger.info(f"{i*1000} reads processed")
...
def polishReads(
    ...
    nanoporeRead = FastaContent(NANOPORE_READ, True)  # build index
    ...
    umiReadDcIter = chunked(sameUmiReadDc.items(), 1000)
    i = 0
    allResults = []

...
def blastMapping(line, blastPath):
    ...
    if os.path.exists(nanoporeFilePath):
        ...
        os.system(f"""{blastPath}makeblastdb -dbtype nucl -in {illuminaFilePath} -hash_index -parse_seqids &&\
...
def windowBlast(ILLUMINA_DIR, NANOPORE_DIR, BLAST_DIR, THREADS, BLAST_PATH):
    ...
    line, blastPath) for line in illuminaFastaList.itertuples())
